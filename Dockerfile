...
CMD ["sh", "-c", "uv run python -m commerce_ops.preflight && uv run alembic upgrade head && uv run python -m commerce_ops.seed_admin && exec uv run uvicorn commerce_ops.main:app --host 0.0.0.0 --port 8000"]
