...
**Product Launch is the MVP subdomain and the first deliverable**, driven from Slack and tracked in ClickUp; marketplace integration is deferred pending external access, so no change is sequenced as depending on a marketplace adapter until it is granted. Launch state is owned three ways — the repository owns the playbook definition, Postgres owns each product's position and per-step completion, ClickUp owns human completion. LangGraph is scoped to interpretation, generation and conversation, not to deterministic rule evaluation. See `README.md`'s Scope, Technology and Architecture sections for the reasoning and the obligations each carries.
