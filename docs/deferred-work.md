...
### The application's global engine singleton outlives pytest's per-test event loop

`database.py`'s `_get_engine_and_session_factory()` is process-wide and
`functools.lru_cache`d by design (its own docstring: "constructed on first
use... not at import time"). Most integration tests never touch it — they
monkeypatch `session`/`transaction` away — but the "live" files that
deliberately exercise the real thing (`test_webhook_advance_atomicity_live.py`,
`test_gate_progression_atomicity_live.py`, and others) call the real,
unpatched accessor, which lazily builds one `AsyncEngine` and keeps its
connection pool for the rest of the pytest process.

That collides with `pytest-asyncio`/`anyio`'s default per-function event
loop: a pooled `asyncpg` connection is bound to the loop it was created
on, and handing it back out to a *later* test running on a *different*
(new) loop fails opaquely — `sqlalchemy.exc.InterfaceError: cannot
perform operation: another operation is in progress` on a `SAVEPOINT`, or
`RuntimeError: Event loop is closed` during teardown, depending on where
in the connection's lifecycle the mismatch lands. Neither error names its
real cause.

**Latent before `trigger-clickup-projection-on-launch-events`, surfaced
reliably by it.** That change added several new real `transaction()` call
sites across this tier (the eager-convergence helper's own lock,
`clickup_sync_job.py`'s restructured per-launch loop, `slack_entry.py`'s
and `gate_confirmation.py`'s eager triggers), which raised how often the
global singleton's pool actually gets exercised during a full run. Verified
by reverting to the commit before that change and rerunning the full
integration suite: the pairing that fails reliably afterward
(`test_clickup_sync_job_containment_live.py` +
`test_slack_entry_start.py`) passed cleanly before, and no other pairing
or full-suite run reproduced it either — the failure does not exist in
isolation or in any two-file combination, only under the accumulated
connection pressure of the full ~130-test run, and which specific test it
lands on shifts with what else runs before it.

**One instance closed, not the mechanism.** `test_clickup_sync_job_containment_live.py`'s
own `driven_job` fixture had a related, more direct bug — it patched
`session` but not `transaction`, so the new lock-acquisition call fell
through to the real global engine even though the rest of that file's
session usage was already properly scoped to the test's own `engine`
fixture. That is fixed (the fixture now binds `transaction` to the same
per-test `engine` too). What remains is the general case: any "live" file
that calls the real accessors at all is exposed to this whenever a full
run's cumulative pool pressure happens to land wrong.

**A fix was attempted and reverted.** `dispose_engine()` already exists
for exactly this shape of problem (`main.py`'s own lifespan calls it on
shutdown) — a function-scoped autouse fixture calling it after every test
would force the next test's first real `session()`/`transaction()` call
to build a fresh engine on its own event loop. Adding that to
`tests/integration/conftest.py` did not work as a five-minute fix: it
produced `pytest`-internal `AssertionError: assert not self._finalizers`
across roughly a sixth of the suite, which reads as an async-fixture/
`anyio`-plugin interaction this project's exact `pytest`+`anyio` version
combination does not accept the way `pytest-asyncio` would — diagnosing
that properly is more than this entry's own change should spend.

**Recorded in**: `trigger-clickup-projection-on-launch-events`'s `tasks.md`
7.1, where the residual flake was found and the baseline-vs-changed
comparison above was run.

**Trigger to close.** A working `dispose_engine()`-after-each-test fixture
(or an equivalent per-test engine reset) added to `tests/integration/conftest.py`
and verified not to break fixture teardown ordering under this project's
`anyio` setup.
