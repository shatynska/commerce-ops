...
- **`PlaybookDefinition`** — versioned, repo-owned (YAML), loaded and validated, never mutated or persisted. A specification object, not an aggregate. Versioning cut-off (recorded 2026-08-23, `complete-playbook-definition`): the shipped `v1` file is edited in place while no real launch has been started against it; from slice 3 on — once a `Launch` records the version it runs under — a definition change means a new version.
  ...
  - Coherence invariants at load: a `prohibited-tactic` never blocks a gate; an `automated` step must carry a rule policy; a description is present, non-empty (whitespace only counts as empty) and single-line; gate positions total and unique. Eight gates: `commit → order → listable → stock-ready → live → ignition → phase-one-complete → graduated` (already in `openspec/specs/launch-playbook`).
  - **The step set is authored** (recorded 2026-08-24, `author-playbook-steps`) — 97 steps, shipped in `v1` edited in place under the cut-off above, since no launch had pinned `v1`. Source is `docs/reference/product-launch.md`: a step's `identifier` is that document's own row ID and its `provenance` is that row's SOURCE citation, so every step traces to exactly one row. The **BUILD THE LISTING area is represented completely** (all 72 ID-bearing rows — 65 on `listable`, 7 reassigned to the gate their own timing serves), because that is where the team's work currently is; the other seven gates carry a **representative subset** sized to exercise every mechanism — all four anchor kinds, all twelve disciplines, both hazards, all three execution modes. Four authoring rules settled with it, each load-bearing rather than stylistic: **hazard is assigned by substance, not by the document's TOS RISK label** — a row naming a tactic to refuse becomes `prohibited-tactic`, a row cautioning against a mistake stays an ordinary step, because a caution is satisfied by heeding it while a tactic can only ever terminate in `Refused`; **blocking is a framework-only structural spine** — given to steps whose absence leaves a gate's meaning unfulfilled rather than to every framework row, with every gate holding at least one; **a row that restates a gate's authored metric condition is not duplicated as a step**, so one obligation is expressed once; and **rule policies stay deliberately incomplete** — two are decided (one `automated`, one `ai-assisted`, the modes that may not ship without one), and the remaining 95 human-attested steps appear in the undecided-rule-policies report, which is how that outstanding decision stays visible instead of looking settled.
...
- **Deferred deliberately**: interactive gate approval in Slack (Block Kit buttons wired to `approve_gate`) waits for `access` to say who may approve — which slice 6a did *not* deliver, having scoped `AccessScope` to visibility and deferred capability flags to the first change that checks one, so approval now waits on that flag rather than on the module existing — a pending confirmation gate is a briefing *item* here, nothing more; and `EvidenceRef` stays briefing-owned rather than moving to `shared`, since the kernel rule is that vocabulary moves up when a second module needs it, not before.
...
The row-level half is realized in slice 6's first change (`introduce-access-scope`, 2026-08-24), with six details worth carrying here because each settles something the map left open:

- **`AccessScope` is kernel vocabulary, not `access`'s own type.** It lives in `shared/domain` from birth: `access` derives it, `catalog` and `launch` take it as a read parameter, `omni_agent` will forward it — the "a second module needs it" rule satisfied on day one. Having `catalog` import a type from `access` would rebuild exactly the coupling briefing's structural ports were introduced to avoid. It is visibility only — unrestricted, or an explicit (possibly empty) set of `ProductId`s, with `permits(product_id)` — and carries no derivation rules, the way `LifecycleStage` carries `is_temporary` while its transitions belong to `catalog`.
- **Unrestricted is a distinct construction, never an enumeration.** The set of all products is unknowable to a value object and grows after a scope is built, so the empty set — the fail-closed default — and "sees everything" are different shapes, not two ends of one list.
- **Capability flags are deferred**, against the map's own sketch above: `AccessScope` carries visibility and nothing else until the first change that actually checks a flag. *Approve gates* arrives with interactive gate approval, itself still deferred from slice 5.
- **Principals are repo-owned YAML granting by SKU.** A small ops team's principals change at code-review cadence, so no table and no admin surface; grants are written by SKU because `ProductId` is opaque and never parsed, and are translated to identifiers at resolution time through a consumer-owned port the composition root satisfies over `catalog`. The shipped file declares an empty list, so no real grant ever ships unreviewed.
- **Resolution never errors toward the asker.** An unknown identity resolves to the empty scope — the same type every resolution yields, never `None` and never a distinct "unknown". A grant naming a SKU no product has confers nothing and is logged, rather than failing the whole resolution: one stale line must not lock a person out of everything they legitimately see. A malformed *directory*, by contrast, is refused at load and validated eagerly at the HTTP composition root, so it stops the deploy instead. Not in `preflight.py`, though a deploy-time check would otherwise belong there — `runtime-configuration` requires that check to read only the process environment, with its outcome depending only on the declared variables.
- **An out-of-scope read is indistinguishable from absence**, in both `catalog` and `launch`: telling them apart would confirm the existence of a product the caller may not see, and would leave callers two absences to handle instead of one. Scope filtering is a separate question from lifecycle filtering and does not reopen it — launch's enumeration is still unfiltered by stage, and now additionally filtered by visibility. Internal processes are not principals: the daily briefing and the ClickUp sync are wired with the unrestricted scope at the composition root, so `briefing` is untouched by the change.
...
| 6a | **`access` scope + scope-aware read use cases**: principals directory, fail-closed resolution, every catalog and launch read filtered by an `AccessScope` — **realized** 2026-08-24 (`introduce-access-scope`) | `access`, `shared` | — |
| 6b | Omni rewired over public surfaces, with adapter guards resolving the asker's scope | `omni_agent`, `access` | needs 6a |
| — | **MVP line** | | |
