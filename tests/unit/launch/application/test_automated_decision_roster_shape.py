"""The shape of the roster collaborator a decision is judged against.
...
unknown identity cannot decide* and *A deactivated person cannot decide*
...
`_FakeRoster`, which answers six read spellings at once and so cannot
observe the shape this delta states. `_ReaderRoster` below answers
`list_people` and nothing else, which is the shape `design.md` —
Decision 2 fixes. That existing file is untouched; `tasks.md` 4.2's
narrowing of `_FakeRoster` is implementation work and is not this
file's.
...
request and no database is needed to see any of them, and the roster
...
- The collaborator's one stated shape, `list_people` (`design.md` —
...
- The spelling by which a message "identifies the shape expected", read
  ...
  `test_authoring_roster_collaborator_shape.py`, which made the same
...
pin `launch.application.UnreadableRosterError`, because `tasks.md` 1.6
...
- `test_an_unknown_identity_cannot_decide` and
  `test_a_deactivated_person_cannot_decide` — expected to **PASS**.
  ...
  new behaviour. Today's implementation resolves the person through a
  three-spelling probe of which `list_people` is one, so the narrowed
...
- The other four tests — expected to **FAIL**. Today `_person_for`
  probes three spellings, finds none on a store-shaped collaborator, and
  returns `None`, so the decision is refused as though the roster did not
...
#: How a refusal may spell "the roster does not carry your identity"
#: (INVENTED). `proposal.md` — *Why* quotes the reply production
#: produces: "the roster does not know that Slack identity".
...
# ---------------------------------------------------------------------------
# Roster collaborators: the stated shape, and the store production injects
# ---------------------------------------------------------------------------
...
class _ReaderRoster:
    ...
    Deliberately narrower than `_FakeRoster` in
...
class _StoreShapedRoster:
    ...
    test_roster_writes.py` records it for `access`'s own writes, and as
    `test_authoring_roster_collaborator_shape.py` records it for the
    sibling call site. It answers nothing about who the roster carries.
    """
...
#: "No roster argument was given", as distinct from "the roster given is
...
def _setup(
    *, roster: Any = _UNSUPPLIED, row: _PendingRow | None = None
) -> _Collaborators:
    playbook = _playbook()
    return _Collaborators(
        results=_FakeResults(row if row is not None else _pending()),
        roster=_reader() if roster is _UNSUPPLIED else roster,
...
def _wiring_error_type() -> type[BaseException]:
    """`UnreadableRosterError`, reached through the module's public
...
async def test_an_unknown_identity_cannot_decide(accept: bool) -> None:
    ...
    collaborator: `_ReaderRoster` answers `list_people` and nothing else,
    ...
    # SPECIFIED (this delta): the roster was actually read. "The roster
    # does not know that Slack identity" is reachable only when the
    # roster was read and did not carry it.
    assert collaborators.roster.reads >= 1, (
        "the decision was refused as unknown without the roster ever being "
...
async def test_a_deactivated_person_cannot_decide(accept: bool) -> None:
    ...
    the full roster including deactivated entries (`design.md` —
    Decision 2), and an implementation that narrowed the read to active
    people alone would pass the test above and fail here.
...
# ---------------------------------------------------------------------------
# Scenario: A collaborator that cannot answer who the roster carries is
# refused by name
# ---------------------------------------------------------------------------
...
async def test_a_collaborator_that_cannot_answer_is_refused_by_name(
    ...
    """Scenario: A collaborator that cannot answer who the roster carries
    is refused by name.
    ...
    *Why* names `load()`/`save()` as what `main.py:140` supplies today.
    ...
    assert outcome.raised is not None, (
        "a collaborator that cannot answer who the roster carries was not "
    ...
    assert type(outcome.raised) is _wiring_error_type(), (
        ...
        "`UnreadableRosterError` that `design.md` — Decision 2 reuses for it"
    )
...
async def test_an_absent_collaborator_is_refused_the_same_way() -> None:
    ...
    """
    absent = _setup(roster=None)
    mis_shaped = _setup(roster=_StoreShapedRoster())
    ...
    assert with_nothing.raised is not None, (
        "a decision at a deployment with no roster collaborator at all was "
...
async def test_a_mis_wiring_is_never_reported_as_an_unknown_identity(
    ...
    The decider here is Alice, whom the roster carries as active. That is
    ...
    vacuously: the same rule, given a readable roster and an identity it
    ...
    store-shaped collaborator is refused with "the roster does not know
    ...
    collaborators = _setup(roster=_StoreShapedRoster())
    ...
    assert not _blames_the_identity(outcome.text), (
        ...
        "decider any reason to believe their roster entry is at fault"
    )
...
async def test_the_shape_check_does_not_move_ahead_of_the_settled_lookup() -> None:
    ...
    does not depend on the roster keeps that refusal."
    ...
    collaborators = _setup(roster=_StoreShapedRoster(), row=settled)
