"""The Postgres members store: the `MembersStore` port over `members`.
...
raising `StaleMembersError` when it has moved. The shape mirrors
...
live in `access.domain.members` and run on every write.
"""
...
from commerce_ops.access.application.members import MemberRecord, StaleMembersError
from commerce_ops.access.domain.members import Member
from commerce_ops.access.infrastructure.driven.models import MemberRow, MembersSet
from commerce_ops.shared.infrastructure.driven.database import session

_SET_ROW_ID = 1


def _record_from_row(row: MemberRow) -> MemberRecord:
    return MemberRecord(
        member=Member(
...
def _row_from_record(record: Any) -> MemberRow:
    member = record.member
    return MemberRow(
        identifier=member.identifier,
        display_name=member.display_name,
        slack_identity=member.slack_identity,
...
class MembersRepository:
    """The membership as stored, behind the port the write use cases hold."""
    ...
    async def _version(self) -> int:
        ...
        if version is None:
            ...
            raise RuntimeError(
                "the membership has no version row — has the migration run?"
            )
        return int(version)

    async def load(self) -> tuple[Sequence[MemberRecord], int]:
        version = await self._version()
        rows = await self._session.scalars(
            select(MemberRow).order_by(MemberRow.display_name)
        )
    ...
    async def save(self, records: Sequence[Any], *, expected_version: int) -> None:
        ...
        if bumped.scalar() is None:
            ...
            raise StaleMembersError(
                f"the membership moved past version {expected_version} while this "
                f"write was validating; reload and revalidate"
            )
        await self._session.execute(delete(MemberRow))
        self._session.add_all(_row_from_record(record) for record in records)
        await self._session.commit()


class PostgresMembers:
    """The `MembersStore` port, opening its own session per operation.
    ...
    async def load(self) -> tuple[Sequence[MemberRecord], int]:
        async with session() as db:
            return await MembersRepository(db).load()

    async def save(self, records: Sequence[Any], *, expected_version: int) -> None:
        async with session() as db:
            await MembersRepository(db).save(records, expected_version=expected_version)
