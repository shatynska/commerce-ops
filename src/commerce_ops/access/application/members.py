"""Write use cases of the `members` capability.

The membership lives in a store behind the `MembersStore` port; these four
operations — plus the startup seed — are the only way it changes. Every
write is validated by constructing the **entire** `Members` the write
would produce, so the same coherence rulebook guards every path, and a
rejected write reports every fault at once (`InvalidMembersError`) while
persisting nothing.

Writes are serialized by the store's optimistic set-version: each
operation loads the membership with its version and persists conditionally on
that version being unchanged. A lost race (`StaleMembersError`) is retried
against the fresh membership, re-validating — the retry may now be rightly
rejected (the second of two deactivations that together would leave the
members without an active admin).

Identifiers are generated, never chosen, and never reused: a step's
assignees reference them, so correcting a member's data must never move
...
"""Exactly what `update_member` may change. The identifier and the Slack
...
class StaleMembersError(RuntimeError):
    """A conditional persist lost the race: the membership changed between
    load and save."""


@dataclass(slots=True)
class MemberRecord:
    """One stored member: their identity data plus the attribution trail.
    ...
    bounded re-assertion turns on whether the membership's only active admin
    ...
    def slack_identity(self) -> str:
        return self.member.slack_identity

    @property
    def active(self) -> bool:
        return self.member.active


class MembersStore(Protocol):
    """The membership persistence port.
    ...
    raising `StaleMembersError` when it has moved.
    """
...
def _as_record(row: Any) -> MemberRecord:
    """A loaded row as a `MemberRecord`, whatever concrete type the store
    yielded — the attribute spellings are the port's contract."""
    if isinstance(row, MemberRecord):
        return row
    return MemberRecord(
        member=row.member,
...
def members_of(rows: Sequence[Any]) -> tuple[Member, ...]:
    """The membership a set of stored rows carries, deactivated included."""
    return tuple(row.member for row in rows)


async def list_members(*, members: MembersStore) -> tuple[Member, ...]:
    """Everyone the membership carries, deactivated included.

    The read half of this capability's public surface, added by
    `redesign-step-fields`: a step's assignees reference members by
    ...
    entries are answered too, because "is this member still active" is a
    question the caller has to be able to ask rather than have decided
    for it.
    """
    rows, _version = await members.load()
    return members_of(rows)


def _validate(rows: Sequence[Any]) -> None:
    """Construct the membership the write would produce; `InvalidMembersError`
    propagates with every fault."""
    Members(members=members_of(rows))


def _generate_identifier() -> str:
    """A fresh identifier, independent of every attribute a member has —
    so correcting their data never moves it."""
    return str(uuid.uuid4())


def _find(rows: Sequence[Any], member_id: str) -> int:
    for index, row in enumerate(rows):
        if row.member.identifier == member_id:
            return index
    raise ValueError(f"no stored member carries the identifier '{member_id}'")


def _copy(row: Any) -> MemberRecord:
...
async def create_member(
    *,
    ...
    """Create a member with a generated identifier, attributed to
    `principal`. Validated as the whole membership it would produce."""
    for _ in range(_WRITE_ATTEMPTS):
        ...
        try:
            ...
            continue
        return record
    raise StaleMembersError(
        f"create_member lost the set-version race {_WRITE_ATTEMPTS} times"
    )
...
async def update_member(
    ...
    **fields: Any,
    ...
    """Update a member's updatable fields — the display name, the ClickUp
    ...
    if "identifier" in fields or "member_id" in fields:
        raise ValueError("a member's identifier is not updatable")
    if "slack_identity" in fields:
        raise ValueError(
            f"a member's Slack identity is not updatable: '{member_id}' keeps "
    ...
    if "active" in fields:
        raise ValueError(
            "active status is not updatable: use deactivate_member or "
            "reactivate_member, so the transition records who made it"
        )
    unknown = sorted(set(fields) - set(_UPDATABLE))
    if unknown:
        raise ValueError(
            f"a member carries no updatable field named {', '.join(unknown)}"
        )
    ...
    for _ in range(_WRITE_ATTEMPTS):
        ...
        was_admin = record.member.admin
        record.member = replace(record.member, **fields)
        if record.member.admin != was_admin:
            record.admin_conferred_by = principal if record.member.admin else None
        ...
        return record
    raise StaleMembersError(
        f"update_member lost the set-version race {_WRITE_ATTEMPTS} times"
    )
...
async def deactivate_member(
    *, members: MembersStore, principal: str, member_id: str
    ...
    for _ in range(_WRITE_ATTEMPTS):
        ...
        index = _find(rows, member_id)
        record = _copy(rows[index])
        record.member = replace(record.member, active=False)
        ...
        return record
    raise StaleMembersError(
        f"deactivate_member lost the set-version race {_WRITE_ATTEMPTS} times"
    )
...
async def reactivate_member(
    *, members: MembersStore, principal: str, member_id: str
    ...
    """Restore a deactivated member under their original identifier,
    attributing the reversal like the deactivation was."""
    for _ in range(_WRITE_ATTEMPTS):
        rows, version = await members.load()
        index = _find(rows, member_id)
        record = _copy(rows[index])
        record.member = replace(record.member, active=True)
        ...
        return record
    raise StaleMembersError(
        f"reactivate_member lost the set-version race {_WRITE_ATTEMPTS} times"
    )


def _seed_conferred(record: MemberRecord) -> bool:
...
async def seed_bootstrap_admin(
    *, members: MembersStore, identity: str | None = None
) -> MemberRecord | None:
    """Ensure the membership has an admin, or explain why it cannot.

    Runs as a preparation step of its own — after the migrations that
    create the members tables, before the server starts — never inside the
    ...
    Four outcomes, exactly as the `members` capability states them:

    - the membership already holds an active admin beyond a lone
      seed-attributed entry — nothing is touched and the variable confers
      nothing;
    - the membership's only active admin is one the seed created and the
    ...
    - the membership holds no active admin and the variable names an identity
      — that identity is created, or promoted if already present, as one
      atomic write;
    - the membership holds no active admin and no variable is set — the step
      fails, naming the variable, and the server never starts.

    The seed is a single create-or-promote save rather than a
    reactivate-then-update pair: every intermediate membership in such a pair
    ...
    active_admins = [
        record for record in records if record.member.admin and record.active
    ]
    ...
    if active_admins:
        ...
        misseeded = (
            ...
            and wanted != active_admins[0].member.slack_identity
        )
        if not misseeded:
            return None
    ...
    if wanted is None:
        raise RuntimeError(
            f"the membership holds no active admin and {BOOTSTRAP_ADMIN_VARIABLE} "
    ...
    existing = next(
        (
            ...
            if record.member.slack_identity == wanted
        ),
        None,
    )
    ...
    if existing is None:
        ...
        seeded.member = replace(seeded.member, admin=True, active=True)
