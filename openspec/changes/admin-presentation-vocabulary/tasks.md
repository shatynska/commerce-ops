...
- [ ] 1.2a Make an **un-injected** `verify` refuse with that same bare
...
- [ ] 1.3 Copy the traversal guard from `playbook_admin.py`'s
  `static_asset` verbatim
  (`path.parent != _STATIC_DIR.resolve() or not path.is_file()`),
  rather than reinventing it.
- [ ] 1.4 Mount the router in `main.py` alongside its neighbours and
  inject `verify` there, next to the existing `roster` /
  `admin_sessions` injection.
- [ ] 1.5 Confirm `uv run lint-imports` passes — `shared` must import no
  business module, and neither admin module gains an import of the
  other.

## 2. The stylesheet

- [ ] 2.1 Add `shared/infrastructure/driving/static/vocabulary.css` with
...
- [ ] 2.2 Add the action vocabulary: `.row-action` for every action
...
- [ ] 2.3 Add the density layer: the action cell as `display: flex` with
  ...
  facts cell.
- [ ] 2.4 Add `.field-faults` treatment and `.just-created` (an untimed
  background tint — no fade on a timer, which no response could assert).
- [ ] 2.5 Ensure `.inapplicable` **dims and never hides** its fieldset,
...
- [ ] 2.5a Do **not** apply `opacity` to `.inapplicable` or to any
...
- [ ] 3.1 Add `shared/infrastructure/driving/templates/_admin_header.html`,
  taking which surface is current and rendering both surfaces, with the
  current one identified rather than linked.
- [ ] 3.2 Give both modules' Jinja `Environment`s a `ChoiceLoader` over
  their own `templates/` plus the shared one — the `_TEMPLATES`
  definition in `playbook_admin.py` and in `roster_admin.py`.
- [ ] 3.3 Include the header on `page.html`, `new.html`, `edit.html` and
  `roster.html`.
- [ ] 3.4 Mark the header's outbound link from the playbook surfaces
...
- [ ] 4.3 Give both action `<td>`s `class="actions"` — the selector
...
- [ ] 4.4 Render the facts cell as `.mark` pills instead of the
  `·`-joined sentence in `page.html`'s `step_cells` macro.
- [ ] 4.5 Add `just-created` to the step row when
...
- [ ] 5.1 Do **not** restructure the anchor groups. An earlier draft of
  ...
  those groups.
- [ ] 5.2 Do not add a rendering-invariant test for marks nested inside
...
- [ ] 5.3 Group the eleven top-level controls into field sections
...
- [ ] 6.1 Delete the inline `<style>` block from `roster.html` and link
  `/admin/assets/pico.min.css` and `/admin/assets/vocabulary.css`.
- [ ] 6.2 Give the page a proper document shell — it currently opens at
  `<!doctype html>` with a bare `<title>` and no `<head>`, `<body>` or
  viewport meta. Do this **before** 6.1 and 6.3, which both assume it.
- [ ] 6.3 Apply the action vocabulary to all four actions the roster
  ...
  row.
- [ ] 6.4 Give the roster's action cells `class="actions"` too, for the
