...
- [ ] 1.5 Define the status transition rule in the domain: any status may move to any other, each move validated by the rules of the status moved *to*, plus the whole-set rules. Define "beyond `draft`" as `in-development` or `active`, excluding `retired`.
- [ ] 1.4 Add the assignee rules — an assignee must be a person the roster carries, and an `active` `human` step needs at least one who is active. The domain cannot read the roster, so it validates against a set of known-active identifiers the caller supplies; the application layer is what fetches them.
...
- [ ] 2.4 Replace `undecided_rule_policies.py` with a report of what blocks activation: the step, its status, and which of brief / handler / active assignee it lacks.
- [ ] 2.5 Take a roster reader as a collaborator so assignee validation can resolve identifiers, supplied by the composition root across the module boundary (`import-linter` forbids `launch` reaching into `access`'s internals).
...
- [ ] 2.6 Retirement now sets `status`; keep `retired_by`/`retired_on` and their un-retire counterparts recording who moved the step and when.
...
- [ ] 3.2 Write the backfill migration: `name` ← `description`, `description` ← null, kind/confirmation ← execution, `status` ← active or retired per the record, `automation_brief` ← `rule_policy`.
...
- [ ] 6.4 Reconcile `playbook-authoring`'s whole-set validation scenario, which currently tests an empty description and a `lesson`-bound blocking step — neither of which remains a fault.
- [ ] 6.2 Reconcile the existing step tests: the seeded-set re-derivation tests move from description to name, the execution-mode vocabulary tests become kind-and-confirmation, and the lesson-cannot-block test goes with the rule. **Rewrite from the new requirement; do not edit assertions to pass.**
- [ ] 6.3 Integration-tier coverage for the migration's backfill against live Postgres.

## 7. Documentation and verification

- [ ] 7.4 Update the capability `Purpose` paragraphs the change's vocabulary outdates — `launch-clickup-sync`'s "human-attested steps" and `playbook-authoring`'s retire/un-retire framing — directly in `openspec/specs/`, since a delta's Purpose is ignored for an existing capability.
- [ ] 7.1 Update `AGENTS.md`'s architecture summary where it describes what a step declares, and `README.md` where it names the step vocabulary.
- [ ] 7.2 Re-sequence `add-step-page` against this field set, or note in its proposal that it is superseded by the form this change specifies.
- [ ] 7.3 Full verification: `uv run pytest`, `ruff check`, `ruff format --check`, `mypy`, `lint-imports`, and both migrations against a local database.
