...
- **The seeded `lp.*` set migrates**: each step's current `description` becomes its `name`, `human-attested` becomes `human`, and `automated`/`ai-assisted` become `automated` with `needs_confirmation` set from which it was. Live `human` steps become `active`; the two automated steps become `in-development`, because nothing registers a handler for them yet and neither blocks a gate. Migrated steps carry no assignees — the readiness report says so rather than an invented owner claiming the work is accepted.
