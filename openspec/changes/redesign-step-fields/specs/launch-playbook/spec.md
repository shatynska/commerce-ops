...
**Reason**: The rule list changes materially and one of its rules ceases to exist. `binding` is removed, so "a lesson may not block its gate" has nothing to check; the execution-mode vocabulary is replaced by kind plus confirmation, so the rule tying automation to a rule policy is restated against status instead; the single-line rule moves from description to name; and the gate-holding floor now counts only active steps. Restated as *An incoherent playbook is rejected against each step's status*, which carries every surviving rule and scenario forward.
**Migration**: None for authors — the same faults are still reported the same way, in one aggregated failure. The rules that changed are named in the replacement requirement.

### Requirement: An undecided rule does not prevent loading

**Reason**: Replaced by the lifecycle status. A missing acceptance criterion no longer needs a permissive load rule, because a step that has not decided one is a `draft` and is not served at all; what may be absent is now a property of the step's status rather than of its execution mode.
**Migration**: Steps that relied on this — human-attested steps with no rule policy — become `human` steps, which carry no automation brief by rule.
