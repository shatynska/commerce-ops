...
**Cadence: `*/10 * * * *`**, matching the ClickUp sync pass.

`*/5` was this decision's first answer, on the grounds that this is the cheapest pass in the system (Postgres reads, no external API calls except an occasional Slack post) and that the interval is what a person waits between a gate becoming ready and being asked about it. Implementation refused it, and the refusal is worth recording rather than quietly resolving: `scheduled-jobs`' longest-gap computation enumerates every occurrence of every schedule over a 400-day horizon and caps that enumeration at 60,000. `*/5` produces 115,200; `*/10` produces 57,600.

The horizon is the load-bearing half — its own comment records that it exists to contain "a full quarterly cycle and both ends of a month-length swing", because the requirement it serves names "weekdays only, or monthly" as the schedules a uniform-gap assumption gets wrong. Raising the cap or shortening the horizon would edit a guard every job depends on, for one job's benefit, which `AGENTS.md`'s scope rule puts in a change of its own. So the schedule moved instead.

What the argument for `*/5` bought is mostly kept: ten minutes is still the fastest cadence in the deployment, and it is well inside "a person waits".
...
This is a pre-existing property of the repository — two recording paths could already clobber each other's step progress — but this change sharpens it, because `current_gate` now actually moves. Accepted for three reasons: the window is milliseconds wide; the next pass re-crosses the gate within ten minutes, so it self-heals; and no gate in this change's scope has an external effect when re-crossed, `graduated` being excluded. Extending the lock to the four recording sites would mean touching every one of them, which is exactly what Decision 1 keeps this change out of.
...
- **A gate opens up to 10 minutes after its conditions are met** → Accepted, Decision 1. Immaterial for a process measured in days; the alternative gives every recording path a Slack notifier it has no business holding, and still misses a gate that becomes ready without an outcome being recorded.
