...
- [x] 6.3 After deploy: confirm `docker logs` shows formatted, timestamped lines
- [x] 6.3a After deploy: trigger the **weekly** cadence by hand with the deployed `TRIGGER_SECRET` and confirm `pending_cadence.py`'s informational record appears — the line that has never reached a log. Do **not** use the 06:00 `daily` cadence for this: it calls `run_daily_digest` and never reaches `pending_cadence.py`, so it cannot produce the record and its absence would read as failure (design.md — Migration Plan)
- [x] 6.4 After deploy: confirm `LOG_LEVEL` is present in the container's environment with the expected value, so the knob is demonstrated to work rather than assumed. This is the verification route for the spec scenario "The threshold can be set in the deployment without changing application code" — there is deliberately no unit test for it, since nothing in `tests/` parses workflow YAML
