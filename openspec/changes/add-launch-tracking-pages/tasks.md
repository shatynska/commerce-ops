...
- [ ] 3.1 Add `launch/infrastructure/driving/launch_admin.py` with the list route at `/admin/launches`, riding the same admin-session guard `roster_admin.py` and `playbook_admin.py` ride; refusal is the app's own 404.
...
- [ ] 3.7 Implement narrowing by gate and by needs-attention, carried in query parameters as `playbook_admin.py` carries its own. A narrowing applies to revealed rows as it does to rows in play, each set narrowing within itself. The reveal control is not itself a narrowing.
- [ ] 3.8 Implement all four empty states distinguishably: no launch position at all; a default view emptied by the filter; a narrowing that matched nothing (which governs when both apply); and revealing when nothing is out of play.
...
- [ ] 4.5 Render each step's name, identifier, discipline, blocking flag, due period, recorded outcome with its provenance, and overdue mark. Render a step with no recorded outcome distinguishably from one recorded not-started.
...
- [ ] 7.1 Run the tests derived from all four delta specs and confirm each scenario is observed.
...
- [ ] 8.2 Open a launch from a row — by clicking, not by typing the URL — and confirm the detail page serves, grouped by gate, landing on the current one.
