...
- **Entries carry the labels the occurrence concerned, captured when it
  ...
  occurrence, not a rendered sentence.
...
behaviour changes; every command keeps its signature, its effect and its
failure modes. The migration is additive.
...
a single append site, and none of those adapters is touched.
...
Also untouched: the ClickUp projection and its intake, the automation
pass and its confirmation path, every authoring write, and every admin
surface.
