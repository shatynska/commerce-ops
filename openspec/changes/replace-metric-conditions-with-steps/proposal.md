...
- **Three gates lose a condition, not one.** `stock-ready` loses `units-fulfillable`; `phase-one-complete` loses `sales-velocity` and `organic-share`; `graduated` loses `tacos` and `review-rating`. Each is replaced by a `draft` step holding nothing until activated, so the first progression pass after deploy may carry a launch further than `stock-ready` — as far as the next gate whose own blocking steps are unresolved, or that requires a confirmation nobody has given.
