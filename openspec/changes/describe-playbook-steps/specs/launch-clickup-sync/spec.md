...
A projected task SHALL be named with the step's description followed by the step's identifier, so that the list states the work while each task remains traceable to the step it stands for. The name SHALL NOT be the sole record of that association: a task renamed or edited in ClickUp SHALL still resolve to its step, because the association is the recorded mapping and never the name.
...
- **THEN** a task named with the step's description followed by its identifier is created in the launch's list
...
- **THEN** the task is created with a shortened name that still carries the step's identifier
