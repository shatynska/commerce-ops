...
9. **Catalog's public surface serves the digest.** `catalog/application/__init__.py` exports the use cases (register product, record ASIN, change stage, get by id/SKU, list); the digest's `ProductNameReader` need is met by the list use case through `products.application` importing `catalog.application` (allowed: cross-module via public surface). The `products`-module repository shrinks to the launch-position record.
