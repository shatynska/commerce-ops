...
- **The scoped `# type: ignore[no-any-return]` at `use_cases.py:28` is removed** once the return type is made concrete (either the function's return path changes to make failure explicit, or the coercion makes the return genuinely `str` without help from an ignore).
