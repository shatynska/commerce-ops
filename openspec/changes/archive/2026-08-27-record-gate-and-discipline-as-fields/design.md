...
- **A shared store this concern cannot restore is the one path where the guarantee is qualified.** → A failed access of the suppression record **on which a report is in flight** — the read deciding whether to report, or the write recording one delivered — obliges a restore before the **first** launch is attempted, where the record shares a session with the per-launch writes. A failure on the *clearing* path does not: it runs on a pass that found no gap, so nothing was reported and nothing later depends on it. Narrowed during implementation, where the unnarrowed rule made a benign clear-path fault end a walk; where the restore itself fails, `One launch's failure does not stop the other launches being converged` gives the ground for ending the walk and failing the run — a ground it states for a failed recovery *between* launches, which this change extends to the pre-walk restore by its own judgement rather than by that requirement's mandate. So a fault of this concern can cost more than the field values, on exactly one path, and the requirement says so rather than leaving the guarantee reading as absolute. The alternative — continue against a store that cannot record — means writing to ClickUp and losing the record of the write, which that requirement judges worse than stopping.
