...
- **A schema addition**: the suppression state and the first-known `known_work` anchor are two tables, each with its own migration. The run history itself is the runner's, and is not duplicated.
