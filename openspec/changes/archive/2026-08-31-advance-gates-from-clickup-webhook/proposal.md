...
- `launch-gate-progression`: the neighboring requirement "A decision records the approval and reports what it did" states "the decision path and the recurring pass SHALL NOT both advance the same launch concurrently" as covering exactly two paths. This change updates that sentence to name the ClickUp webhook's trigger as a third path bound by the same one-at-a-time rule, so the requirement's own text stays accurate once this change ships rather than describing a system that no longer exists.

## Impact

- **Code**: `src/commerce_ops/launch/infrastructure/driving/clickup_webhook.py` (new trigger call, background-task dispatch), likely a small shared helper extracted from `gate_progression_job.py`'s `_advance_one`/`_ask_if_owed` so the webhook and the periodic pass call the same cascade rather than duplicating it. `clickup_webhook.py`'s own docstring documents its collaborators as bare-global imports specifically so tests can substitute fakes with `monkeypatch.setattr`; the new cascade import into that module needs the same treatment (named in `__all__`, referenced as a bare global) to stay consistent with that convention and testable the same way.
