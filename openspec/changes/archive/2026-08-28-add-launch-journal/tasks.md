...
- [x] 11.3 **Corrected after the fact — the prediction here was wrong.** This task expected the pages change's two `xfail(strict=True)` journal tests to start failing the moment the journal merged. They do not, and should not. That change merged first (643a01d) with its own task 4.8 still open, leaving `launch_admin.read_journal: Any = None` as an injection point the detail page never calls — it renders `journal=(), journal_available=False` unconditionally. The seam therefore *resolves* (so `_journal_seam` finds it) while the page still ignores it, and both tests keep xfailing. Verified after merging `origin/main` into this branch: `1384 passed, 2 xfailed`.

  What actually unblocks them is the pages change's task 4.8 — wiring `launch_admin.py` to call the read and render the section. **That is not this change's work** and is deliberately not done here (AGENTS.md — implement only what belongs to the change in progress). It is now unblocked, and is the follow-up this change hands over.
