...
Both are `<select multiple>`. That control deselects only on ctrl/cmd-click: a plain click *replaces* the whole selection with the option clicked, so once an author has chosen anything there is no plain-click route back to none. For `after_steps` none is the **ordinary** value — most steps wait on nothing — so an author who picked a step by accident has no way to undo it, and a step that should wait on nothing cannot be returned to that state once anything is chosen. There is no partial fix in place: the control as it ships has no plain-click route back to the empty set at all.
