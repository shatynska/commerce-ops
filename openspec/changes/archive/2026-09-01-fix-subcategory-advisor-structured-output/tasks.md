...
Carried out on 2026-09-01 against image `47aef7b` (the merge commit), deployed 06:25Z. Recorded here rather than only in a conversation, since this section is the only place that says whether the change actually worked.

- [x] 4.1 Confirm an automation pass actually ran after the deploy, from the run record rather than from a log grep: `curl -sS https://fuperia.shatynska.com/health/scheduled-runs` and check that `launch.automation.resolution_pass`'s `last_success` is later than the deploy. A log grep cannot serve as this gate — the pass emits no line on success, only warnings on failure (`automation_pass.py` logs at `warning` and nothing else), so the token would match only the failures the fix removes, and a zero count would be indistinguishable from a pass that never ran.
      **Verified**: passes at `06:30:16Z` and `06:45:03Z`, both after the 06:25Z deploy.
- [x] 4.2 With a pass confirmed to have run, check `docker compose logs --since <deploy> worker | grep -c "Unsupported function"` is `0`, **and** that `lp.listing.007` actually resolved on a product. A pass that logs no failure but records nothing has not been verified — that is the shape the "no verdict could be read" path produces, and it is the outcome a schema the model fills inconsistently would give.
      **Verified**: `Unsupported function` → `0`, `failed resolving step` → `0`. The step resolved on real products, not merely stopped failing: `Disposable food trays 33` at `06:30:01Z` and `Disposable food trays 32` at `06:45:00Z`, both `satisfied` from `listing.subcategory_advisor`, each carrying a node path, its compliance demands and a rejected alternative. The `TestProductName*` launches recorded `blocked` with the model's own reason ("insufficient information about the product"), which is the unsupported route working, not a failure.
- [x] 4.3 If the API rejects the schema despite both local conversions accepting it (`design.md`'s first risk), the error will name the schema rather than the union; that is a new finding for this change, not a separate one.
      **Verified**: no rejection. `design.md`'s first risk — the one no offline check could rule out — did not materialise; the flat schema is accepted by the API as well as by both local conversions.

Two facts established by this verification that were previously only inferred, recorded because a later reader will otherwise re-derive them:

- **A step stays released past its own gate.** `Disposable food trays 32` passed the `listable` gate at 06:32:01Z, 38 seconds after reaching it, because `lp.creative.008` — the gate's only *blocking* active step — was ticked in ClickUp 116 ms earlier. `lp.listing.007` is non-blocking, so nothing waited for it, and the `*/15` pass had no window to run it. It resolved at the next pass anyway, confirming `has_released`'s "at or beyond, never by equality" (`launch_run.py:700`) behaves as its docstring says. A launch advancing past an unresolved automated step is a normal, visible state, not a fault.
- **The gap between a gate opening and the step resolving is bounded by the pass cadence**, up to ~15 minutes. Whether the advisor's answer ought to exist *before* the gate opens is a playbook authoring question — making the step blocking — not a code change, and is deliberately not proposed here.
