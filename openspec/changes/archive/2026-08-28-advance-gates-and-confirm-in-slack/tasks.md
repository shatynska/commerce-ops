...
- [x] 7.5 Before the first run against a deployment, count the launches `list_active` returns there and how many stand at a satisfied confirmation gate — the first pass takes an empty cool-off table, so every one of them is asked in a single run, and no requirement caps asks per run. **Measured 2026-08-28 against production: 3 launches walked, all at `commit`; exactly 1 (`d8ccbed7`) asked about on the first run — no pre-seeding of the cool-off table warranted**
- [x] 7.6 Verify by hand against a test launch: satisfy a `commit` step, watch the pass ask in Slack, approve, and confirm the gate opens and the journal records the approval and the crossing. **Verified 2026-08-28 against production on `d8ccbed7`: ask delivered 16:52:00, approval recorded 16:55:00.824, `gate-opened` 16:55:01.406 (582 ms later — the inline advance, not the next pass), `current_gate` now `order`**
