...
`report_undecided_rule_policies(playbook)` takes a loaded `LaunchPlaybook` and returns frozen rows (identifier, gate, discipline, execution mode). *(Amended during implementation: the original wording had the use case load via the existing loader itself, but `.importlinter`'s module-layers contract forbids the application layer from importing infrastructure — so the caller loads and hands the playbook in, the same consumer-side wiring `catalog` already uses for its ports.)* No delivery channel in this change — the v1 playbook has zero steps, so the report is empty until step authoring begins, which is exactly when its consumer (the authoring workflow) appears. Public-surface placement follows the Omni rule: if Omni later answers "which steps are undecided?", this use case is its tool.
