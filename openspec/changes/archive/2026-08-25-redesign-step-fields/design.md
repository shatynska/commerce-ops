...
The same principle settles the handler, and the first draft of this design failed to apply it there. That a handler is *present* is a property of the step set and is checked at load; that the deployed code *registers* it is a property of the deployment, which changes without the step set changing — so registration is checked when a step is activated, and a deploy that drops a handler an active step names is reported at startup rather than making every playbook load fail. A rename in the registry should fail a deployment, not take down launches.

*Alternative considered*: give the load path a roster reader, and a registry reader, too. Rejected — it makes every playbook read depend on another module's store being reachable and on the registry's current contents, turning a roster edit or a rename into a launch-wide outage.
...
- **[Migrated steps are active and unowned]** → Deliberate, and visible: they appear in the readiness report until someone accepts them, and ClickUp assignee reconciliation means assigning one later reaches its already-projected task. This only works because the assignee preconditions are evaluated over the steps a write *touches*, not over the whole resulting set — set-wide, 95 unowned steps would refuse every subsequent write until all 95 were assigned, which is the backfill the product decision declined. The residue is real and worth stating: editing a migrated step will require giving it an owner before it saves.
