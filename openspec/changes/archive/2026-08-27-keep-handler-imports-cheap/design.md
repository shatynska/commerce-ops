...
- **An import failure moves from process start to first invocation.** Today a broken or incompatible `langchain_openai` fails when `commerce_ops.main` is imported — loudly, before the container is healthy. After this change it fails inside `build_production_graph()`, during a pass, where `launch-step-automation`'s *A handler failure resolves nothing and does not stop the pass* turns it into a per-step fault rather than a refused start. → Accepted, and worth naming since Decision 1 rejects the lazy-proxy alternative partly on failure-timing grounds: the weaker form of that objection applies to the accepted option too. The residual case is a packaging fault, and dependencies are locked and baked into the image, so it cannot arise from configuration or a runtime upgrade. It now surfaces as a reported handler failure, which the pass already logs with a traceback.
