...
- [x] 9.3 Confirm the migration applies cleanly against a local database and `launch_positions` gains both columns. — reran against a local Postgres (`docker compose up -d postgres`): `alembic upgrade head` applied cleanly (`d5f83b1e04a7` → `028812c68321` → `1a2b3c4d5e6f`), `seed_playbook` ran, and the full suite passed for real — `1890 passed, 50 skipped` across unit + agents + integration, including all 5 of `tests/integration/launch/test_slack_entry_start.py`'s tests (the anchor-date and ClickUp-cadence assertions added while closing out task 8.3 included).
