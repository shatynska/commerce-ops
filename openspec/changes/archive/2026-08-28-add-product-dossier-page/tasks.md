...
- [x] 9.1 Exercise both pages by hand: the index lists in SKU order within each group; a row opens its dossier; the header offers the index from the playbook and roster surfaces and back. **The retired-row rule was not observable**: the deployment holds two products, neither retired, so "set apart and following the rest" has no case to render. Covered by `test_product_index_page.py`'s *Setting apart outranks the SKU sort*; recorded in `docs/deferred-work.md` rather than left as an open task
- [x] 9.2 Open the dossier for a product with no launch and one mid-launch — the first states that nothing has been produced, the second renders its record. Both confirmed against the deployment on 2026-08-28. **No graduated launch exists**, so that third case was not observable; recorded in `docs/deferred-work.md`. Note that the no-launch record is empty *by construction* — `automated_step_results.product_id` is a foreign key to `launch_positions`, so a product with no launch can carry no result
- [x] 9.3 Confirm a pending entry reads as awaiting a decision, carries no decider and offers no decision control — confirmed against the deployment on 2026-08-28. **`accepted`, `rejected` and `voided` were not observable**: the deployment held exactly one retained result, and it was pending. Reaching the other three costs at least the 24-hour rejection cool-off, and `voided` needs a step de-activated between a result being produced and its decision. All four states are asserted by `test_product_dossier_page.py`; recorded in `docs/deferred-work.md`
- [x] 9.4 Confirm both pages 404 identically with no session cookie, and that the stylesheet does too
