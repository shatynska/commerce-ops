...
**8. The YAML seeds once, then retires — with the seed's source vendored into the migration.**
An Alembic data migration inserts the authored step rows and validates them by constructing `LaunchPlaybook` over `framework_gates()` — the same rulebook as every load and write, so the migration cannot insert what the domain would reject. *Refined during implementation:* the migration parses its **own vendored copy** of the authored file (`alembic/data/playbook_v1.yaml`) with migration-local parsing, rather than importing the source-tree loader — because a fresh environment (new developer database, CI) runs `alembic upgrade head` from scratch *after* the follow-up commit removes the YAML and loader from the source tree, and a migration importing deleted code would strand every future bootstrap. The follow-up commit still removes `playbook_v1.yaml`, `playbook_loader.py`'s YAML path, and `shipped_playbooks.py` from `src/`; the vendored copy is the migration's asset, not a source of truth.
...
- [The seed migration must outlive the deletion of the YAML and loader it seeds from] → the seed's source is vendored into `alembic/data/` and parsed by the migration itself (decision 8), so a fresh environment bootstraps identically before and after the deletion. Rollback of the schema migration restores nothing YAML — the source file additionally stays in git history.
