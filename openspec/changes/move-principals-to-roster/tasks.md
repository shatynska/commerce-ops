...
- [x] 3.4 Declare `BOOTSTRAP_ADMIN_IDENTITY` (optional) in the runtime-configuration definition, and implement the seeding step through the validated write path, as its own process between `alembic upgrade head` and the server (`commerce_ops.seed_admin`), never in the lifespan: an unreadable store fails the step; a store with an active admin beyond a lone seed-attributed entry → touch nothing; readable, admin-less + variable → upsert that identity as active admin (display name = the identity) attributed to `system:bootstrap`; readable with only the single seed-attributed admin + variable naming a different identity → seed the new identity, deactivating nothing; admin-less with no variable → the step fails naming the variable, and the server never starts.
...
- [x] 5.8 Integration-tier coverage the unit tier cannot observe: the Postgres roster store including the stale-version race (mirroring `tests/integration/launch/test_playbook_authoring_live.py`), and the lifespan actually calling the bootstrap step. Written as `tests/integration/access/test_roster_store_live.py` (round trip with attribution, identifier stability across an edit, the stale-version race, deactivation-as-flag). The lifespan *calling* the seed is left to the deploy itself: asserting it needs a process boundary this tier has no harness for.
...
- [x] 6.4 **Decided: option (a), the pre-serving step.** Verification found a conflict this change never wrote a delta for: `database-session` requires the connection setting to be read *no earlier than the first session request*, and the lifespan seed reads it at startup. It is a real violation, not only a test artefact — though the test that catches it is `tests/integration/shared/test_scheduled_runs_freshness_unreachable.py`, whose two cases fail because the seed caches an engine built from a working URL before the test can repoint `DATABASE_URL` at an unanswering address. Proven by experiment: disabling the lifespan seed makes both pass, restoring it fails them again. (The separate full-tier hang is **not** from this change — it reproduces with the seed disabled, and belongs to `verify-the-integration-tier`.) Options: (a) move the seed to an explicit deploy step beside `alembic upgrade head`, which keeps the app's lazy-connection guarantee and fails the deploy loudly rather than crash-looping; (b) keep it in the lifespan and add a `database-session` delta admitting one startup read; (c) run it lazily on first admin-surface use, which drops the "startup refuses" requirement. Option (a) was taken and the specs revised: (b) leaves the behaviour unchanged so the test still fails, and (c) trades away the loud failure. The container chain is now `preflight && alembic upgrade head && seed_admin && exec uvicorn`; `commerce_ops.seed_admin` is the step, the lifespan touches nothing, and `tests/unit/test_startup_without_configuration.py` gained a guard asserting startup builds no engine — verified to fail when the lifespan seed is reintroduced. The two freshness tests pass unedited.
