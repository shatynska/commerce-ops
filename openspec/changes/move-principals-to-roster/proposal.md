...
- **Bootstrap**: when the roster is readable and holds no active admin at startup, the identity named by a declared environment variable is seeded as an active admin person — solving first-boot without a permanent out-of-band authority. An unconfigured or unreachable store defers the seed to the next start rather than blocking startup, preserving `runtime-configuration`'s empty-environment startup guarantee and `database-session`'s lazy connection rule.
