...
Two existing capabilities bind the new entrypoint, and the point of naming them here is that neither had to be amended to accommodate it: `database-session` requires any process that obtained a session to close its pool before exiting, and `application-logging` requires every entrypoint — non-HTTP ones included — to configure logging before its work. A step added to this chain later inherits both.
