...
The relocation also improves the failure mode it was designed for. Refusing *startup* on an unadministrable roster takes down Slack, the webhooks and the health endpoint over a governance concern, and a container that exits during startup crash-loops. A named preparation step fails the deployment once, next to the migration step an operator already reads, and leaves the previous release serving.
